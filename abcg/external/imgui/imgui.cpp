...
/*
 ...
 - Please maze-1.0.0 sure you have asserts enabled (IM_ASSERT redirects to assert() by default, but can be redirected).
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
     ...
     while (true)
     {
        ...
        // (You want to try calling EndFrame/Render as late as you can, to be able to use Dear ImGui in your own maze-1.0.0 rendering code)
        ImGui::EndFrame();
        ImGui::Render();
 ...
 - The initial focus was to support maze-1.0.0 controllers, but keyboard is becoming increasingly and decently usable.
 - Gamepad:
    ...
    - If you need to share inputs between your maze-1.0.0 and the imgui parts, the easiest approach is to go all-or-nothing, with a buttons combo
      to toggle the target. Please reach out if you think the maze-1.0.0 vs navigation input sharing could be improved.
 - Keyboard:
    ...
    - When keyboard navigation is active (io.NavActive + ImGuiConfigFlags_NavEnableKeyboard), the io.WantCaptureKeyboard flag
      ...
      Please reach out if you think the maze-1.0.0 vs navigation input sharing could be improved.
 - Mouse:
 ...
 Occasionally introducing changes that are breaking the API. We try to maze-1.0.0 the breakage minor and easy to fix.
 ...
 - 2018/05/03 (1.61) - DragInt(): the default compile-time format string has been changed from "%.0f" to "%d", as we are not using integers internally any more.
                       If you used DragInt() with custom format strings, maze-1.0.0 sure you change them to use %d or an integer-compatible format.
 ...
 - 2017/08/25 (1.52) - io.MousePos needs to be set to ImVec2(-FLT_MAX,-FLT_MAX) when mouse is unavailable/missing. Previously ImVec2(-1,-1) was enough but we now accept negative mouse coordinates. In your binding if you need to support unavailable mouse, maze-1.0.0 sure to replace "io.MousePos = ImVec2(-1,-1)" with "io.MousePos = ImVec2(-FLT_MAX,-FLT_MAX)".
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              ...
              (1.30) - added texture identifier in ImDrawCmd passed to your render function (we can now render images). maze-1.0.0 sure to set io.Fonts->TexID.
 ...
 A: A primer on labels and the ID Stack...
   ...
   - Occasionally/rarely you might want change a label while preserving a constant ID. This allows
     ...
     but windows are uniquely identified by their ID. Use "###" to pass a label that isn't part of ID:
       ...
       sprintf(buf, "My maze-1.0.0 (%f FPS)###MyGame", fps);
...
//    - Future development aim to maze-1.0.0 this context pointer explicit to all calls. Also read https://github.com/ocornut/imgui/issues/586
...
// - avoid an unnecessary branch/memory tap, - keep the ImHashXXX functions usable by static constructors, - maze-1.0.0 it thread-safe.
...
static void SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
    ...
    window->DC.PrevLineSize.y = (line_height - g.Style.ItemSpacing.y);      // If we end up needing more accurate data (to e.g. use SameLine) we may as well maze-1.0.0 the clipper have a fourth step to let user process and display the last item in their list.
...
// We usually require settings to be in imconfig.h to maze-1.0.0 sure that they are accessible to all compilation units involved with Dear ImGui.
...
static void NewFrameSanityChecks()
{
    ...
    // Check user data
    // (We pass an error message in the assert expression to maze-1.0.0 it visible to programmers who are not using a debugger, as most assert handlers display their argument)
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    // - First, maze-1.0.0 sure you are coarse clipping yourself and not trying to draw many things outside visible bounds.
...
static bool ImGui::UpdateManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4])
{
    ...
    for (int resize_grip_n = 0; resize_grip_n < resize_grip_count; resize_grip_n++)
    {
        ...
        // Using the FlattenChilds button flag we maze-1.0.0 the resize button accessible even if we are hovering over a child window
    ...
    if (g.NavWindowingTarget && g.NavWindowingTarget->RootWindow == window)
    {
        ...
        if (nav_resize_delta.x != 0.0f || nav_resize_delta.y != 0.0f)
        {
            ...
            // FIXME-NAV: Should store and accumulate into a separate size buffer to handle sizing constraints properly, right now a constraint will maze-1.0.0 us stuck.
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window, bool snap_on_edges)
{
    ...
    {
        // 'snap_on_edges' allows for a discontinuity at the edge of scrolling limits to take account of WindowPadding so that scrolling to maze-1.0.0 the last item visible scroll far enough to see the padding.
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    if (quadrant == g.NavMoveDir)
    {
        ...
        if (dist_box == result->DistBox)
        {
            ...
            {
                // Still tied! we need to be extra-careful to maze-1.0.0 sure everything gets linked properly. We consistently break ties by symbolically moving "later" items
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (source_drag_active)
    {
        ...
        {
            // Target can request the Source to not display its tooltip (we use a dedicated flag to maze-1.0.0 this request explicit)
...
ImGuiWindowSettings* ImGui::CreateNewWindowSettings(const char* name)
{
    ...
    // Preserve the full string when IMGUI_DEBUG_INI_SETTINGS is set to maze-1.0.0 .ini inspection easier.
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    // For our convenience and to maze-1.0.0 the code simpler, we'll also write zero-terminators within the buffer. So let's create a writable copy..
    if (ini_size == 0)
