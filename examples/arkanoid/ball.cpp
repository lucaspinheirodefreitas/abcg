...
bool Ball::verifyPosition(Paddle &paddle) {
    return paddle.m_translation.x-0.12<=m_translation.x && paddle.m_translation.x+0.12>=m_translation.x;
}

void Ball::update(Paddle &paddle, const GameData &gameData) {
    if (gameData.m_state == State::Playing) {
        if (m_bulletCoolDownTimer.elapsed() > 60.0 / 1000.0) {
            ...
            if (m_start) {
                ...
                m_translation.x -= 0.005f;
            } else {
                if (m_esquerda) {
                    if(m_translation.x < -0.0f) {
                        m_translation.x - 0.05f < -0.99f ? m_translation.x = -0.99f : m_translation.x-= 0.05f;
                    } else {
                    ...
                    if(m_translation.x < -0.0f) {
                        ...
                        m_translation.x + 0.05f < 0.99f ? m_translation.x += 0.05f : m_translation.x = 0.99f;
                    }
                ...
                if (m_baixo) {
                    if(m_translation.y < -0.0f) {
                        ...
                        ? m_translation.y-= 0.05 : (verifyPosition(paddle) ? m_translation.y = -0.87f : m_translation.y-= 0.05);
                    } else {
                    ...
                    if(m_translation.y < -0.0f) {
                        ...
                        m_translation.y + 0.05f < 0.99f ? m_translation.y += 0.05f : m_translation.y = 0.99f;
                    }
                }
            }
        }
