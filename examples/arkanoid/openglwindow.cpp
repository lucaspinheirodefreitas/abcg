...
void OpenGLWindow::restart() {
    ...
    m_rectangle.initializeGL(m_objectsProgram, 10);
...
/*void OpenGLWindow::checkDefense() {
    if (m_ball.m_translation.y<-0.88f + 0.01f + 0.04f) {
        if(m_ball.m_translation.x >= m_paddle.m_translation.x - 0.12f && m_ball.m_translation.x <= m_paddle.m_translation.x + 0.12f) {
            m_paddle.m_defense = true;
            cout << "defense: " << m_ball.m_translation.y << endl;
        }
    }
}*/
